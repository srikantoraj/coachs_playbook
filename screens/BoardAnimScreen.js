...
import { SafeAreaView, Dimensions, StyleSheet, Image, View, PanResponder, TouchableOpacity, ToastAndroid, Modal, Text  } from 'react-native';
...
export default function BoardAnimScreen({navigation, route}) {
    ...
    const panResponder = useRef(
        ...
        onPanResponderMove: (event, gestureState) => {
            ...
            if(!isShare) {
                ...
                bezierPoints.push({x : realX, y : realY});
    ...
    const onVideoControlBtn = (key) => {
        ...
        if(key == 'stadium') {
            ...
            if(game.stageIdx == 0) return;
            game.stages.splice(game.stageIdx, 1);
            game.stageIdx--;
        } else if(key == 'before') {
            if(game.stageIdx == 0) return;
            game.stageIdx--;
            ...
            game.stageIdx++;
            game.stageIdx = game.stageIdx >= game.stages.length? game.stages.length - 1 : game.stageIdx;
        } else if(key == 'play') {
            game.isPlaying = !game.isPlaying
            updatePlay();

        } else if(key == 'video') {
            game.isVideoPlaying = !game.isVideoPlaying;
        } else if(key == 'zoomIn') {
            console.log(game)
        ...
        // if(!game.stages[game.stageIdx].paths) {
        //     game.stages[game.stageIdx].paths = [];
        // }
        // if(!game.stages[game.stageIdx].items) {
        //     game.stages[game.stageIdx].items = [];
        // }

        generateDrawItems();


        // console.log(items, game.stageIdx,paths)

        setStageIdx(game.stageIdx);

        drawCanvas()
    }
    
    const saveStage = (type = 'add') => {
        ...
        if(type == 'add') {
            if(game.stages.length == 0) {
                ...
                let newStage = [
                    ...game.stages.slice(0, game.stageIdx+1), 
                    {
                        paths: [...paths],
                        items: [...getItemsJSON(type)],
                    }, 
                    ...game.stages.slice(game.stageIdx+1)];
                game.stages = [...newStage];
            }
    ...
    const getItemsJSON = (type) => {
        let json = [];
        items.forEach(item => {
            if(type == 'add' || item.isNew) {
                item.startPos = {
                    x : item.x,
                    y : item.y
                }
        ...
        if (ref.current) {
            ...
            const draw = () => {
                ...
                items.forEach((item, i) => {
                    ...
                    ctx.setLineDash([5, 5]);
                    ctx.lineWidth = 1;
                    ctx.moveTo(item.startPos.x + game.camera.x, item.startPos.y + game.camera.y);
                    ctx.lineTo(item.x + game.camera.x, item.y + game.camera.y);
                    ctx.stroke();
                    ctx.setLineDash([]);
                    ctx.closePath();
                })
            ...
            window.updatePlay = () => {
                ...
                items.forEach((item, i) => {
                    let x = item.startPos.x + (item.x - item.startPos.x) * (curTime / during)
  ...
  return (
    <SafeAreaView style={{ flex: 1 }}>
        ...
        <View style={styles.header_down}>
            <TouchableOpacity onPress={onVideoControlBtn.bind(this, 'delete')}>
                <Image style={styles.image} source={require('../images/icons/delete.png')} />
            </TouchableOpacity>
            <TouchableOpacity onPress={onVideoControlBtn.bind(this, 'add')}>
                <Image style={styles.image} source={require('../images/icons/add.png')} />
            </TouchableOpacity>
            <TouchableOpacity onPress={onVideoControlBtn.bind(this, 'before')}>
                <Image style={styles.image} source={require('../images/icons/before.png')} />
            </TouchableOpacity>
            <TouchableOpacity onPress={onVideoControlBtn.bind(this, 'next')}>
                <Image style={styles.image} source={require('../images/icons/next.png')} />
            </TouchableOpacity>

            <TouchableOpacity onPress={onVideoControlBtn.bind(this, 'play')}>
                <Image style={styles.image} source={require('../images/icons/stop.png')} />
            </TouchableOpacity>
            <TouchableOpacity style={styles.image}  onPress={onVideoControlBtn.bind(this, 'video')}>
                <StyledText bold>
                    {stageIdx}
                </StyledText>
            </TouchableOpacity>
            <TouchableOpacity onPress={onVideoControlBtn.bind(this, 'video')}>
            ...
            {/* <TouchableOpacity style={styles.image} onPress={onVideoControlBtn.bind(this, 'stadium')}>
                <Image source={require('../images/icons/football-1.png')} />
            </TouchableOpacity> */}

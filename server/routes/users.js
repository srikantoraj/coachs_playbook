...
router.post('/change_password', function(req, res, next) {
  const { password, verifyPassword, id, email } = req.body
  ...
  User.getUserByEmail(email, function (error, user) {
    ...
    if (user) {
        ...
        User.updateUserById(user._id, newUser, function(err, user1) {
        
          console.log("---------update----------")

          return res.json({ message: 'user password is changed.' })
        })
